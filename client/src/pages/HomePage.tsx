...
import { ArrowRight, DoorOpen, Headphones, Users, Timer, Trophy, Shuffle, Code2, ListMusic, Sparkles, Radio, UserRound } from "lucide-react";
    ...
    <div className="home-screen screen-pad min-h-screen">
      ...
      <div className="home-layout">
        <motion.section initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:.38}} className="home-action game-panel order-1 lg:order-2">
          ...
          <form className="flex flex-col gap-3" onSubmit={(e)=>{e.preventDefault();if(code.trim().length===5 && serverReady && serverConnected) onJoin(normalizeCode(code));}}>
            ...
            <Button type="submit" variant="secondary" size="lg" disabled={code.trim().length!==5 || !serverReady || !serverConnected} className="w-full"><DoorOpen size={19}/>Entrar na sala</Button>
          </form>
          <div className="solo-hint"><Headphones size={15}/><span>Prefere jogar solo? Crie uma sala e comece sem esperar ninguém.</span></div>
        ...
        <motion.section initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:.05,duration:.42}} className="home-brand order-2 lg:order-1">
          ...
          <div className="home-features">
            ...
            <span className="home-pill"><ListMusic size={15}/> Catálogo 900+</span>
